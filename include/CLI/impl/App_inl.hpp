...
template <class CharT> CLI11_INLINE void App::parse_char_t(int argc, const CharT *const *argv) {
    ...
    */
    ...
    CLI11_INLINE void App::_process_help_flags(CallbackPriority priority, bool trigger_help, bool trigger_all_help)
        const {
    ...
    CLI11_INLINE bool App::_add_flag_like_result(
        Option * op, const ConfigItem &item, const std::vector<std::string> &inputs) {
    ...
    CLI11_NODISCARD CLI11_INLINE App *App::_find_subcommand(
        const std::string &subc_name, bool ignore_disabled, bool ignore_used) const noexcept {
        App *bcom{nullptr};
    ...
    CLI11_INLINE bool App::_parse_arg(
        std::vector<std::string> & args, detail::Classifier current_type, bool local_processing_only) {
    ...
    CLI11_NODISCARD CLI11_INLINE const std::string &App::_compare_subcommand_names(const App &subcom, const App &base)
        const {
