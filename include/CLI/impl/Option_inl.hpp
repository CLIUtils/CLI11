...
CLI11_NODISCARD CLI11_INLINE const std::string &Option::matching_name(const Option &other) const {
    ...
    for(const std::string &sname : snames_) {
        if(other.check_sname(sname))
            return sname;
        if(configurable && other.check_lname(sname))
            return sname;
    }
    for(const std::string &lname : lnames_) {
        if(other.check_lname(lname))
            return lname;
        if(lname.size() == 1 && configurable) {
            if(other.check_sname(lname)) {
                return lname;
            }
        }
    }
    if(configurable && snames_.empty() && lnames_.empty() && !pname_.empty()) {
        if(other.check_sname(pname_) || other.check_lname(pname_) || pname_ == other.pname_)
            return pname_;
    }
    if(configurable && other.snames_.empty() && other.fnames_.empty() && !other.pname_.empty()) {
